...
for Num_users_index=1:length(Num_user_cluster) % Number of users
    ...
    for Num_RF_index = 1:length(RF_sets)
        ...
        TX_index = 0;
        for TX_ant=16  %Number of UPA TX antennas
            TX_index = TX_index+1;
            ...
            for K = [2]
                ...
                for Num_paths_index=1:length(paths) %Number of channel paths
                    ...
                    Rate_dc = zeros(1,length(SNR_dB_range));
...
%                             Rate_BS_BDMA(SNR_index)=Rate_BS_BDMA(SNR_index)+PGH(Heff_p,p,sigma)/(ITER);
                            ...
                            Rate_HP_SLNR(SNR_index) = Rate_HP_SLNR(SNR_index) + RGH(G_cl,Fbb_slnr,rho)/(ITER);
                             ...
                             %% --------------dc programming-----------%%
                         %Rate_dc(SNR_index) = Rate_dc(SNR_index) +  blockDCP(Frf_cl, G_cl, Num_users, rho, Fbb_cl);
                      %   Rate_dc(SNR_index) = Rate_dc(SNR_index) +  zffunc(Frf_cl, G_cl, Num_users, rho);
                            %%---------------dc 
                        end % End of SNR loop
...
hold on; %plot(SNR_dB_range,Rate_dc,'linewidth',1.5);
